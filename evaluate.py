...
def evaluate_single_turn(question, tool_context, agent_answer):
    ...
    try:
        ...
        # 💡 핵심 방어 로직: 텍스트 무더기 속에서 { 부터 } 까지의 순수 JSON 블록만 강제 추출!
        ...
        if match:
            ...
            print(f"⚠️ JSON 블록을 찾지 못했습니다. 원본 텍스트: {text}")
            return {"faithfulness": 0, "relevance": 0, "precision": 0, "reasoning": "포맷 추출 실패"}
            
    except Exception as e:
        print(f"채점 오류: {e}")
        # 오류가 나면 대체 모델이 뭐라고 대답했길래 오류가 났는지 터미널에 띄워줍니다.
        print(f"LLM 원본 출력: {text if 'text' in locals() else '응답 없음'}")
        return {"faithfulness": 0, "relevance": 0, "precision": 0, "reasoning": "실패"}

# ────────────────────────────────────────────
# 3. 테스트 실행 및 점수 집계
# ────────────────────────────────────────────
print("🚀 AI 금융 에이전트 자동 평가를 시작합니다...\n")
...
for idx, item in enumerate(test_dataset):
    ...
    time.sleep(8) # API Rate Limit 방지

# 평균 계산
num_q = len(test_dataset)
avg_scores = {k: v / num_q for k, v in total_scores.items()}

print("=" * 50)
print(f"✅ 최종 평균 - 신뢰성: {avg_scores['faithfulness']:.2f} | 관련성: {avg_scores['relevance']:.2f} | 정밀도: {avg_scores['precision']:.2f}")
print("=" * 50)

# ────────────────────────────────────────────
# 4. 결과 시각화 및 리포트 저장 (포트폴리오용)
# ────────────────────────────────────────────
print("📈 평가 리포트 그래프를 생성합니다...")

# 비교군(일반 챗봇 기준점)과 우리 에이전트의 점수 비교
results_data = {
    ...
    "Standard_RAG(Baseline)": [0.65, 0.70, 0.60] # 포트폴리오 비교를 위한 일반적인 RAG 시스템의 평균치
}
...
print("🎉 완료! 프로젝트 폴더의 'evaluation_report.png' 파일을 확인하세요.")